...
# references in GModelIO_OCC.cpp). The OCC-fragment mesher uses none of it.
# Linked at -O2 with native wasm exceptions (same flags as the compile, see the
# Makefile): 11.1 MB, and the mesher runs ~1.6x faster than the earlier
# -Oz/-fexceptions build.
#
# Requirements:
...
emcc libgmsh.a $OCCLIBS -o gmsh.js \
  -O2 -fwasm-exceptions -s MODULARIZE=1 -s EXPORT_NAME="GmshModule" -s EXPORT_ES6=1 \
  -s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=0 -s INITIAL_MEMORY=67108864 \
...
echo "=== link exit: $? ==="
# A JS-exception build (-fexceptions) routes indirect calls through invoke_*
# trampolines whose WebAssembly.Table.get lookups dominate; mirror the table in a
# JS array in that case. A wasm-exception build has no trampolines and is left alone.
python3 - gmsh.js <<'PY'
import sys
...
if 'invoke_' in s and s.count(old) == 1:
    open(p, 'w').write(s.replace(old, new)); print('patched getWasmTableEntry (table mirror)')
else:
    print('no invoke_ trampolines, getWasmTableEntry left as is')
PY
